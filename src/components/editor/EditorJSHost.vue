...
import MathQuillComponent from "@/components/editor/MathQuillComponent.vue";
import CalculatorTool from "@/components/editor/CalculatorTool.vue";
import patchEditorJS from "@/editorjs-patches";
import _ from "@/util";
import { ONoteSocket } from "@/socket";
import CodeMirrorTool from "@/components/editor/CodeMirrorTool.vue";
...
export default class EditorJSHost extends Vue {
  ...
  scroller: PerfectScrollbar | null = null;
  ...
  async editorDidLoad() {
    ...
    const redactor: HTMLDivElement = this.getModule("UI").nodes.holder;
    this.scroller = new PerfectScrollbar(redactor);

    redactor.addEventListener("resize", () => this.scroller && this.scroller.update());

    this.$watch("scrollLock", scrollLock => {
      const UI = this.getModule("UI");
      const { wrapper } = UI.nodes;
      wrapper.style.overflow = scrollLock ? "hidden" : "initial";
    }, { immediate: true });
  ...
  receiveCRUD({action, data: { uuid, data }}: CRUDPacket) {
    ...
    var tool: InstanceType<ReturnType<typeof toolForVueComponent>>, newBlock;
    switch (action) {
    case "create":
      if (uuid === null) {
      ...
      break;
    case "replace":
      newBlock = BlockManager.composeBlock(data["toolName"], data["data"], {}, uuid);
      BlockManager._blocks.insert(basisIndex, newBlock, true);
      break;
    case "update":
      switch (data["toolName"]) {
      case "math":
        tool = BlockManager.getBlockByIndex(basisIndex).tool;
      ...
      newBlock = BlockManager.composeBlock(data["toolName"], data["data"]["data"], data["settings"], data["settings"]["__uuid__"]);
      BlockManager._blocks.insert(basisIndex, newBlock, true);
      break;
    case "delete":
  ...
  createEditor(data?: any) {
    ...
    this.editor = new EditorJS({
      ...
      onChange: () => this.save(),
      onClick: (e) => this.$root.$emit("click", e),
  ...
  async save(id: string = this.noteID) {
    ...
    await this.$store.dispatch("updateNote", { id, data });
  }
